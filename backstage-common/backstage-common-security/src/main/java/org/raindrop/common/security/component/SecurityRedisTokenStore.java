 ...
 */
public class SecurityRedisTokenStore implements TokenStore {
	...
	public SecurityRedisTokenStore(RedisConnectionFactory connectionFactory) {
	...
	private byte[] serializeKey(String object) {
		return serialize((prefix + object) + StrUtil.COLON);
	}
	...
	public long doMaintenance() {
		...
		try (RedisConnection conn = getConnection()) {
			...
			Cursor<byte[]> clientToAccessKeys = conn.scan(ScanOptions.scanOptions()
					.match((prefix + CLIENT_ID_TO_ACCESS + "*" + StrUtil.COLON)).build());
			...
			Cursor<byte[]> unameToAccessKeys = conn.scan(ScanOptions.scanOptions()
					.match(prefix + UNAME_TO_ACCESS + "*" + StrUtil.COLON).build());
